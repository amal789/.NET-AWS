...
-- this parameter from SPUPDATEFIRMWARESERIALNUMBER. to get only serial number detatils 
...
FROM dbo.VCUSTOMER WITH ( NOLOCK ) WHERE USERNAME = @USERNAME 

SELECT @ENABLEORGBASEDASSET = APPLICATIONCONFIGVALUE FROM APPLICATIONCONFIGVALUE (NOLOCK) WHERE APPLICATIONCONFIGNAME='ENABLEORGBASEDASSET' 
...
SELECT @MOBILESERVERPRODUCTLISTORGANIZATIONID = APPLICATIONCONFIGVALUE FROM APPLICATIONCONFIGVALUE WITH (NOLOCK) WHERE APPLICATIONCONFIGNAME='MOBILESERVERPRODUCTLISTORGANIZATIONIDS' 

SELECT @RECENTCOUNT=CAST(APPLICATIONCONFIGVALUE AS int) FROM APPLICATIONCONFIGVALUE WITH (NOLOCK) WHERE APPLICATIONCONFIGNAME='MOBILESERVERPRODUCTLISTCOUNT' 

IF EXISTS( SELECT USERNAME FROM vcustomer WHERE ORGANIZATIONID IN (SELECT * FROM FNSPLITCSV(@MOBILESERVERPRODUCTLISTORGANIZATIONID) ) AND USERNAME = @USERNAME) 
BEGIN 
...
EXECUTE SP_GETMESSAGESTRING 'ASSOWITH', @LANGUAGECODE, 1, @ASSOWITHTXT OUTPUT 
...
SELECT PG.PARTYGROUPID , PG.PARTYGROUPNAME, P.CONTACTID, PTG.PRODUCTGROUPID, PTG.PRODUCTGROUPNAME, SERIALNUMBER, P.ORGANIZATIONID, PTG.ADMINPARTYID 
into #tempPRGD 
from PARTYGROUP PG (nolock) , PARTYGROUPDETAIL PGD (nolock) , PARTYPRODUCTGROUP PPG (nolock) , PRODUCTGROUP PTG (nolock) , PRODUCTGROUPDETAIL PTGD (nolock) , PARTY P (nolock) 
...
IF @ASSOCTYPE ='CHILDASSOCIATE' AND @ASSOCTYPEID IN ( SELECT PRODUCTASSOCIATIONTYPEID FROM PRODUCTASSOCIATIONTYPE WITH (NOLOCK) WHERE INTERNALDESCRIPTION='FIREWALL_CONNECTOR') 
BEGIN 
...
BEGIN 
INSERT INTO #TEMPLISTTABLE ( PRODUCTID , SERIALNUMBER , PRODUCTFAMILY, PRODUCTLINE, ACTIVEPROMOTION ) 
SELECT TOP (@RECENTCOUNT) CP.PRODUCTID , CP.SERIALNUMBER , CP.PRODUCTFAMILY 'PRODUCTFAMILY', PRODUCTLINE, ACTIVEPROMOTION 
FROM CUSTOMERPRODUCTSSUMMARY CP WITH ( NOLOCK ) 
WHERE CP.SERIALNUMBER IN ( SELECT SERIALNUMBER FROM #tempPRGD) 
AND CP.USEDSTATUS = 1 
...
INSERT INTO #TEMPLISTTABLE ( PRODUCTID , SERIALNUMBER , PRODUCTFAMILY , PRODUCTLINE, ACTIVEPROMOTION ) 
SELECT TOP (SELECT CAST(APPLICATIONCONFIGVALUE AS int) FROM APPLICATIONCONFIGVALUE WITH (NOLOCK) WHERE APPLICATIONCONFIGNAME='MOBILESERVERPRODUCTLISTCOUNT') CP.PRODUCTID , CP.SERIALNUMBER , CP.PRODUCTFAMILY 'PRODUCTFAMILY', PRODUCTLINE, ACTIVEPROMOTION 
FROM CUSTOMERPRODUCTSSUMMARY CP WITH ( NOLOCK ) 
WHERE CP.USERNAME = @USERNAME 
and CP.SERIALNUMBER NOT IN ( SELECT SERIALNUMBER FROM #tempPRGD) 
AND CP.USEDSTATUS = 1 
AND CP.PRODUCTFAMILY NOT IN ( 'CLIENTLICENSE','FLEXSPEND' ) 
AND CP.PRODUCTLINE NOT IN ('STORAGE MODULE','SATA MODULE','NSspTenant','M2 STORAGE MODULE' ) 
END 
...
BEGIN 
INSERT INTO #TEMPLISTTABLE ( PRODUCTID , SERIALNUMBER , PRODUCTFAMILY, PRODUCTLINE, ACTIVEPROMOTION ) 
SELECT CP.PRODUCTID , CP.SERIALNUMBER , CP.PRODUCTFAMILY 'PRODUCTFAMILY', PRODUCTLINE, ACTIVEPROMOTION 
FROM CUSTOMERPRODUCTSSUMMARY CP WITH ( NOLOCK ) 
WHERE CP.SERIALNUMBER IN ( SELECT SERIALNUMBER FROM #tempPRGD) 
AND CP.USEDSTATUS = 1 
AND CP.PRODUCTFAMILY NOT IN ( 'CLIENTLICENSE','FLEXSPEND' ) 
...
-- Continue with rest of stored procedure logic...
-- The file appears to be cut off in the user's message
END