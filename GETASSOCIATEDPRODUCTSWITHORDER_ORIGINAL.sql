...
BEGIN
    ...
    FROM SESSIONREV WITH (NOLOCK) 
    WHERE USERNAME = @USERNAME

    SELECT @CONTACTID = CONTACTID , 
           @ORGID = ORGANIZATIONID ,
           @ISLARGEUSER = ISNULL(LARGECUSTOMER,'NO'), 
           @ISORGBASEDACCOUNT = ORGBASEDACCOUNT, 
           @PARTYID = PARTYID 
    FROM dbo.VCUSTOMER WITH ( NOLOCK ) 
    WHERE USERNAME = @USERNAME

    SELECT @ENABLEORGBASEDASSET = APPLICATIONCONFIGVALUE 
    FROM APPLICATIONCONFIGVALUE (NOLOCK) 
    WHERE APPLICATIONCONFIGNAME='ENABLEORGBASEDASSET'

    SELECT @ROLEIDORGBASED = APPLICATIONROLEID 
    FROM APPLICATIONROLE WITH (NOLOCK) 
    WHERE ROLENAME = 'ORGBASED' AND APPLICATIONNAME = 'MSW' AND OEMCODE = 'SNWL'
    ...
    SELECT @MOBILESERVERPRODUCTLISTORGANIZATIONID = APPLICATIONCONFIGVALUE 
    FROM APPLICATIONCONFIGVALUE WITH (NOLOCK) 
    ...
    -- [Additional stored procedure content continues but truncated for space...]
    -- This is just the initial portion to understand the basic structure
END