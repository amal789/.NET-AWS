...
SELECT @CONTACTID = CONTACTID , @ORGID = ORGANIZATIONID ,@ISLARGEUSER = ISNULL(LARGECUSTOMER,'NO'), @ISORGBASEDACCOUNT = ORGBASEDACCOUNT, @PARTYID = PARTYID FROM dbo.VCUSTOMER WITH ( NOLOCK ) WHERE USERNAME = @USERNAME
...
SELECT @ORGBASEDASSETOWNSERSHIPENABLED = CASE WHEN ISNULL(@ENABLEORGBASEDASSET,'') = 'YES' THEN 'YES'
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            ...
            INSERT INTO #TEMPLISTTABLE ( PRODUCTID , SERIALNUMBER , PRODUCTFAMILY , PRODUCTLINE, ACTIVEPROMOTION )
            SELECT TOP (SELECT CAST(APPLICATIONCONFIGVALUE AS int) FROM APPLICATIONCONFIGVALUE WITH (NOLOCK) WHERE APPLICATIONCONFIGNAME='MOBILESERVERPRODUCTLISTCOUNT')
                CP.PRODUCTID ,
                CP.SERIALNUMBER ,
                CP.PRODUCTFAMILY 'PRODUCTFAMILY',
                PRODUCTLINE,
    ...
    BEGIN
        ...
        BEGIN
            ...
            WHERE CP.SERIALNUMBER IN ( SELECT SERIALNUMBER FROM #tempPRGD)
                ...
                AND CP.USEDSTATUS = 1
                AND CP.SERIALNUMBER IN (
                    SELECT CHILDSERIALNUMBER FROM dbo.DEVICEASSOCIATION WITH ( NOLOCK )
                    WHERE PRODUCTASSOCIATIONTYPEID IN ( 5, 6, 70 )--@ASSOCTYPEID
                        AND PRIMARYSERIALNUMBER = @SERIALNUMBER
                        AND @ASSOCTYPE = 'CHILDASSOCIATE'
                    UNION
                    SELECT PRIMARYSERIALNUMBER FROM dbo.DEVICEASSOCIATION WITH ( NOLOCK )
                    WHERE PRODUCTASSOCIATIONTYPEID IN ( 5, 6, 70 )--@ASSOCTYPEID
            ...
            SELECT TOP (SELECT CAST(APPLICATIONCONFIGVALUE AS int) FROM APPLICATIONCONFIGVALUE WITH (NOLOCK) WHERE APPLICATIONCONFIGNAME='MOBILESERVERPRODUCTLISTCOUNT')
                ...
                ACTIVEPROMOTION
            FROM CUSTOMERPRODUCTSSUMMARY CP WITH ( NOLOCK )
            WHERE CP.SERIALNUMBER NOT IN ( SELECT SERIALNUMBER FROM #tempPRGD)
                AND CP.PRODUCTFAMILY NOT IN ( 'CLIENTLICENSE','FLEXSPEND' )
                AND CP.PRODUCTLINE NOT IN ('STORAGE MODULE','SATA MODULE','NSspTenant','M2 STORAGE MODULE' )
                AND CP.USEDSTATUS = 1
                AND CP.SERIALNUMBER IN (
...
ELSE IF ISNULL(@ASSOCTYPEID, 0) <> 0 AND ISNULL(@ASSOCTYPEID, 0) <> 70
BEGIN
    IF ISNULL(@SEARCHSERIALNUMBER,'') = ''
    BEGIN
        IF ((ISNULL(@APPLICATIONNAME,'') = 'MSWANDROID' OR ISNULL(@APPLICATIONNAME,'') = 'MSWIOS') AND @ISSELECTEDPARTNER=1)
...
-- The rest of the stored procedure continues with updates and final processing...
-- [The file appears to be truncated in the user's message, but this gives us the key logic for comparison]
